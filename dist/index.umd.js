(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.LOGO_PARTICLE = {}, global.PIXI));
  ...
  class Stage {
    ...
    _normalizeImage(url, config) {
      const resource = PIXI__namespace.Loader.shared.resources[url];
      const image = new PIXI__namespace.Sprite(resource.texture);
      const origin = {
        width: image.width,
        height: image.height
      };
    ...
    async loadImage(url, options) {
      ...
      return new Promise((resolve) => {
        if (PIXI__namespace.Loader.shared.resources[url]) {
          const posList = this._normalizeImage(url, config);
          resolve(posList);
        } else {
          PIXI__namespace.Loader.shared.add(url).load(() => {
            const posList = this._normalizeImage(url, config);
            resolve(posList);
          });
        }
